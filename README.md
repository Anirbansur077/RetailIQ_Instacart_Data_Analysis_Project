# 📌 Project Progress

## Phase 1: Business Understanding
- [x] Downloaded Instacart dataset
- [x] Understood business problem
- [x] Identified project objectives
- [x] Created business questions

## Phase 2: Data Understanding
- [x] Understood all tables
- [x] Identified Primary Keys
- [x] Identified Foreign Keys
- [x] Created ER Diagram
- [x] Created Data Dictionary

## Phase 3: Data Quality Assessment
- [x] Checked dataset dimensions
- [x] Checked data types
- [x] Checked missing values
- [x] Checked duplicate rows
- [x] Checked unique values

## Phase 4: Data Cleaning
- [x] Removed unnecessary columns
- [x] Converted data types
- [x] Documented cleaning decisions

## Phase 5: Exploratory Data Analysis
- [x] Univariate Analysis
- [x] Bivariate Analysis
- [x] Multivariate Analysis

## Phase 6: SQL Analysis
- [x] Database creation
- [x] Business queries
- [x] Advanced SQL

## Phase 7: Power BI Dashboard
- [ ] Data model
- [ ] KPIs
- [ ] Interactive dashboard

## Phase 8: Documentation
- [ ] Executive report
- [ ] Final README
- [ ] GitHub screenshots